...
class CryptoTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(self.state[self.state_interval * self.crypto_dim + 1 : (self.state_interval + 1) * self.crypto_dim + 1]), np.array(self.num_crypto_shares)) # current timestamp portfolio value
    ...
    def _initiate_state(self):
        if self.crypto_dim > 1:
            ...
            state = (
                [self.cash] + self.num_crypto_shares + [self.data.close] + [self.data.open] + [self.data.high] + [self.data.low] + [self.data.volume]
            )
        return state
    ...
    def _update_state(self):
        if self.crypto_dim > 1:
            state = (
                [self.state[0]] + list(self.state[1 : (self.crypto_dim + 1)]) + self.data.close.values.tolist() + self.data.open.values.tolist() + self.data.high.values.tolist() + self.data.low.values.tolist() + self.data.volume.values.tolist()
            ) 
        else:
            state = (
                [self.state[0]] + list(self.state[1 : (self.crypto_dim + 1)]) + [self.data.close] + [self.data.open] + [self.data.high] + [self.data.low] + [self.data.volume]
            ) 
        return state
    ...
    def _sell_share(self, index, action):
        if self.state[index + 1] > 0:
            sell_num_shares = min(abs(action) / self.state[index + self.state_interval * self.crypto_dim + 1], self.state[index + 1])
            sell_amount = (self.state[index + self.state_interval * self.crypto_dim + 1] * sell_num_shares * (1 - self.sell_cost_pct[index]))
            self.state[0] += sell_amount
            self.state[index + 1] -= sell_num_shares
            self.cum_sell_cost += (self.state[index + self.state_interval * self.crypto_dim + 1] * sell_num_shares * self.sell_cost_pct[index])
            self.sell_trades += 1
        else:
            sell_amount = 0
    ...
    def _buy_share(self, index, action):
        if self.state[0] > 0:
            ...
            buy_num_shares = buy_amount / (self.state[index + self.state_interval * self.crypto_dim + 1] * (1 + self.buy_cost_pct[index]))  
            self.state[0] -= buy_amount
            self.state[index + 1] += buy_num_shares
            self.cum_buy_cost += (self.state[index + self.state_interval * self.crypto_dim + 1] * buy_num_shares * self.buy_cost_pct[index])
            self.buy_trades += 1
        else:
            buy_amount = 0
    ...
    def _reward_setting(self, terminal):
        # reward shaping
        if terminal: reward = np.log(self.portfolio_values_memory[-1] / self.portfolio_values_memory[0]) * self.timestamp / self.data_granularity
            ...
            if self.timestamp > self.eval_time_interval and self.risk_control:
                short_term_reward = np.log(self.portfolio_values_memory[-1] / self.portfolio_values_memory[- self.eval_time_interval + self.state_interval]) * self.alpha * self.eval_time_interval / self.data_granularity
                max_downturn_flag = (self.portfolio_values_memory[-1]/self.max_portfolio_value) < (1 - self.max_downturn_threshold)
                ...
                if (self.max_portfolio_value < self.portfolio_values_memory[-1]): self.max_portfolio_value = self.portfolio_values_memory[-1]
        return reward
    ...
    def step(self, actions):
        ...
        if self.immediate_sell == 1:
            actions = np.zeros(self.crypto_dim) - self.portfolio_value # sell all crypto at once if max downturn threshold is reached
            self.immediate_sell = 0
            self.max_portfolio_value = self.portfolio_values_memory[-1]
        self.actions_memory.append(actions)
        self.portfolio_value = self._calculate_portfolio_value(self.state[0], np.array(self.state[(self.state_interval * self.crypto_dim + 1) : ((self.state_interval + 1) * self.crypto_dim + 1)]), np.array(self.state[1 : 1 + self.crypto_dim]))
            ...
            if self.episode % self.print_verbosity == 0 and self.model_name != "":
                ...
                if self.is_debug:
                    ...
                    try:
                        ...
                        df_valid_actions.to_csv(f"../../results/valid_actions/{self.model_name}/episode_{self.episode}.csv")
                        df_rewards.to_csv(f"../../results/rewards/{self.model_name}/episode_{self.episode}.csv")
                        df_portfolio_value.to_csv(f"../../results/portfolio_value/{self.model_name}/episode_{self.episode}.csv")
    ...
    def reset(self):
        ...
        self.portfolio_value = self._calculate_portfolio_value(self.cash, np.array(self.state[self.state_interval * self.crypto_dim + 1 : (self.state_interval + 1) * self.crypto_dim + 1]), np.array(self.num_crypto_shares))
        self.cum_buy_cost = 0 
